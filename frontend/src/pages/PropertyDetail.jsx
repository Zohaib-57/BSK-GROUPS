...
const FALLBACK_AGENT = {
	...
	avatar:
		"https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=256&h=256",
};
...
function OverviewTab({ property }) {
	return (
		<div>
			...
			{/* Amenities */}
			{property.features &&
				Object.values(property.features).some((v) => v === true) && (
					<div style={card}>
...
function LocationTab({ property }) {
	const q = encodeURIComponent(
		`${property.location?.address}, ${property.location?.city}, Pakistan`,
	);
	return (
		<div style={card}>
	...
	return (
		<div style={card}>
			<h2 style={sectionTitle}>
				Price Index — {property.area?.value} {property.area?.unit} in{" "}
			...
			>
				...
				<div>
					...
					<div style={{ fontSize: 20, fontWeight: 700, color: "#16a34a" }}>
						+PKR {formatPrice(current - prev)} ({change}%)
					</div>
				</div>
				{[
					["6 months ago", mockData[6].price],
					["12 months ago", mockData[3].price],
					["24 months ago", mockData[0].price],
				].map(([label, val]) => (
					<div key={label}>
						<div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 2 }}>
							{label}
				...
				{["6 Months", "1 Year", "Max"].map((t) => (
					<button
						...
						style={{
							...
							fontSize: 12,
							cursor: "pointer",
							background: t === "Max" ? "#C8A45A" : "#fff",
							color: t === "Max" ? "#fff" : "#64748b",
						}}
					>
						{t}
					</button>
...
function TrendsTab({ property }) {
	...
	return (
		<div style={card}>
			<h2 style={sectionTitle}>
				Trends — Most Searched Locations in {property.location?.area || "DHA"}
			</h2>
				...
				>
					<thead>
						<tr style={{ background: "#f8fafc" }}>
							{["Rank", "Locality", "% of Total Searches", "Performance"].map(
								(h) => (
									<th
										key={h}
										style={{
							...
							>
								...
								<td style={{ padding: "10px 14px" }}>
									...
									>
										...
										<span style={{ color: "#475569", minWidth: 36 }}>
											{t.pct}%
										</span>
									</div>
								</td>
...
export default function PropertyDetailPage() {
	...
	const [formData, setFormData] = useState({
		...
		phone: "",
		message:
			"I would like to inquire about this property. Please contact me at your earliest convenience.",
		role: "buyer",
	});
	...
	const displayAgent =
		property.agent && property.agent.role !== "admin"
			? property.agent
	...
	const handleInquiry = async (e) => {
		...
		try {
			await inquiryAPI.create({
				...
				message:
					formData.message ||
					`I would like to inquire about ${property.title}. Please contact me at your earliest convenience.`,
				role: formData.role,
			});
					...
					{[1, 2, 3].map((i) => (
						...
						>
							...
							{i === 3 && images.length > 4 && (
								...
								>
									<Images size={28} />+{images.length - 4} Photos
								</div>
							)}
						</div>
					))}
			...
			<div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0" }}>
				...
				>
					{property.bedrooms > 0 && (
						...
						>
							<Bed size={16} style={{ color: "#C8A45A" }} />{" "}
					...
					{property.bathrooms > 0 && (
						...
						>
							<Bath size={16} style={{ color: "#C8A45A" }} />{" "}
					...
					>
						<Square size={16} style={{ color: "#C8A45A" }} />{" "}
						<strong>{property.area?.value}</strong> {property.area?.unit}
					</div>
			...
			<div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0" }}>
				...
				>
					...
					<div style={{ textAlign: "right" }}>
						...
						<div style={{ fontSize: 26, fontWeight: 800, color: "#C8A45A" }}>
				...
				<aside style={{ position: "sticky", top: 60 }}>
					...
					>
						...
						<div style={{ display: "flex", gap: 10 }}>
							<a
								href={`https://wa.me/${displayAgent.phone?.replace(/\D/g, "")}`}
								target="_blank"
								rel="noreferrer"
								style={{
									flex: 1,
						...
						>
							...
							<div style={{ display: "flex", alignItems: "center", gap: 12 }}>
								<img
									src={
										displayAgent.avatar ||
										`https://ui-avatars.com/api/?name=${displayAgent.name}&background=C8A45A&color=fff`
									}
								...
								<div>
									...
									<div
										style={{ fontSize: 12, color: "#C8A45A", fontWeight: 600 }}
									>
										{displayAgent.role || "Property Expert"}
									</div>
								</div>
								<span
									style={{
					...
					>
						<ShieldCheck size={14} style={{ color: "#C8A45A" }} /> BSK Secure
						Transaction
					</div>
				</aside>
			</div>
			...
			>
				<div style={{ maxWidth: 1200, margin: "0 auto" }}>
					...
					<p style={{ fontSize: 13, color: "#64748b", marginBottom: 12 }}>
						For your safety and protection when buying, selling, or renting
						property through BSK Groups:
					</p>
					<ul
						style={{
							paddingLeft: 20,
					...
					>
						...
						<li>
							...
							and completion of all legal formalities.
						</li>
						<li>
							Inspect the property thoroughly and ensure all details match the
							advertisement.
						</li>
						<li>
							Verify ownership documents through relevant authorities before
							finalizing any deal.
						</li>
					</ul>
				</div>
